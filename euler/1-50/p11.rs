//!
//! In the $20 \times 20$ grid below, four numbers along a diagonal line have
//! been marked in red. (<7, 9, 26>, <8, 10, 63>, <9, 11, 78>, <10, 12, 14>)
//!
...
//!
//! # 题目描述
//! 找到前后左右（包括斜角）相邻的4个数的积的最大值
//!
//! # 题解
//! - DFS，注意不要多次计算同一个位置上的数字，可以限制访问方向来控制
//!   比如只允许 左、左上、上、右上 这个四个方向 （需要考虑 T 型的情况）
//!
//! # 答案
//! 70600674
//!
fn main() {
  ...
  for r in 1..ROW {
    for c in 1..COL {
      let t = dfs(&num_map, &directions, r, c, 1, 4);
      if answer < t {
        answer = t;
      }
    }
...
fn dfs (
  ...
  let ct = t * (num_map[r][c] as i64);
  let mut answer: i64 = ct;

  // Optimization: Zero times anything is zero
  if answer == 0 {
    return answer;
  }
  ...
  for direction in directions.iter() {
    let nr:i32 = (r as i32) + direction[0];
    let nc:i32 = (c as i32) + direction[1];
    if nr < 0 || nr >= (num_map.len() as i32) {
      continue;
    }

    let row: &Vec<i32> = &num_map[nr as usize];
    if nc >= 0 && nc < (row.len() as i32) {
      let nt = dfs(num_map, directions, nr as usize, nc as usize, ct, cur - 1);
      flag = true;
      if answer < nt {
        answer = nt;
      }
